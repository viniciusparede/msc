...
def create_dataset(words, labels) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    words_keys = set(list(words.keys()))
    labels_keys = set(list(labels.keys()))

    keys = list(words_keys & labels_keys)

    train_keys = list(set(keys) & set(STANDARD_TRAIN_KEYS))
    valid_keys = list(set(keys) & set(STANDARD_VALID_KEYS))
    test_keys = list(set(keys) & set(STANDARD_TEST_KEYS))

    train_dfs = [open_sentence(words, labels, key) for key in train_keys]
    valid_dfs = [open_sentence(words, labels, key) for key in valid_keys] 
    test_dfs = [open_sentence(words, labels, key) for key in test_keys]
